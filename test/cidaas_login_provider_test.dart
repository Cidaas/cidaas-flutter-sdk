...
void main() {
  ...
  group('CidaasLoginProvider tests, user is not logged in & correct config:',
    ...
    setUp(() {
      ...
      when(storage.write(
              key: AuthStorageHelper.ACCESS_TOKEN,
              value: AuthStorageHelper.ACCESS_TOKEN))
          .thenReturn(null);
      when(storage.write(
              key: AuthStorageHelper.SUB, value: AuthStorageHelper.SUB))
          .thenReturn(null);
      when(storage.write(
              key: AuthStorageHelper.ID_TOKEN,
              value: AuthStorageHelper.ID_TOKEN))
          .thenReturn(null);
      when(storage.write(
              key: AuthStorageHelper.REFRESH_TOKEN,
              value: AuthStorageHelper.REFRESH_TOKEN))
          .thenReturn(null);
      when(storage.read(key: AuthStorageHelper.ACCESS_TOKEN))
          .thenAnswer((_) => Future<String>(() => null));
      when(storage.read(key: AuthStorageHelper.SUB))
          .thenAnswer((_) => Future<String>(() => null));
      when(storage.read(key: AuthStorageHelper.ID_TOKEN))
          .thenAnswer((_) => Future<String>(() => null));
      when(storage.read(key: AuthStorageHelper.REFRESH_TOKEN))
          .thenAnswer((_) => Future<String>(() => null));
      ...
      when(httpMock.get('https://baseUrl.de/.well-known/openid-configuration',
              headers: anyNamed('headers')))
          .thenAnswer((_) =>
              Future<http.Response>(() => http.Response(jsonString, 200)));
      when(httpMock.post('https://nightlybuild.cidaas.de/token-srv/token',
    ...
    test('load config & getConfig', () async {
      final OpenIdConfiguration idConfig =
          await CidaasLoginProvider.checkAndLoadConfig();
      expect(idConfig != null, true);
      expect(idConfig.issuer, 'https://nightlybuild.cidaas.de');
      final OpenIdConfiguration idConfigLater =
          await CidaasLoginProvider.getOpenIdConfiguration();
      expect(idConfigLater != null, true);
      expect(idConfigLater.issuer, 'https://nightlybuild.cidaas.de');
      final CidaasConfig cidaasConfig =
          await CidaasLoginProvider.getCidaasConf();
    ...
    test('get access_token by code', () async {
      final TokenEntity tokenEntity =
          await CidaasLoginProvider.getAccessTokenByCode('code');
    ...
    test('get renewAccessTokenByRefreshToken', () async {
      final TokenEntity tokenEntity =
          await CidaasLoginProvider.renewAccessTokenByRefreshToken(
              'refreshToken');
      expect(tokenEntity != null, true);
      expect(tokenEntity.accessToken, 'ReturnedAccessToken');
      expect(tokenEntity.idToken, 'ReturnedIdToken');
  ...
  test('get getTokenClaimSetForToken', () {
    final Map<String, dynamic> tokenClaims =
        CidaasLoginProvider.getTokenClaimSetForToken(
  ...
  test('load config with http instead https', () {
    runZoned(() {
      CidaasLoginProvider.checkAndLoadConfig(
          configPath: 'assets/cidaas_config_http.json');
  ...
  test('load config without clientId', () {
    runZoned(() {
      CidaasLoginProvider.checkAndLoadConfig(
          configPath: 'assets/cidaas_config_without_clientId.json');
    }, onError: expectAsync1((Exception e) {
      expect(e is ConfigurationError, true);
      expect(e.toString(),
          'ConfigurationError: clientId is not set in cidaas config: assets/cidaas_config_without_clientId.json');
    }));
  });
  ...
  test('load config without clientSecret', () {
    runZoned(() {
      CidaasLoginProvider.checkAndLoadConfig(
          configPath: 'assets/cidaas_config_without_clientSecret.json');
    }, onError: expectAsync1((Exception e) {
      expect(e is ConfigurationError, true);
      expect(e.toString(),
  ...
  test('load config without redirectUri', () {
    runZoned(() {
      CidaasLoginProvider.checkAndLoadConfig(
          configPath: 'assets/cidaas_config_without_redirectUri.json');
    }, onError: expectAsync1((Exception e) {
      expect(e is ConfigurationError, true);
      expect(e.toString(),
  ...
  test('load config without baseUrl', () {
    runZoned(() {
      CidaasLoginProvider.checkAndLoadConfig(
          configPath: 'assets/cidaas_config_without_baseUrl.json');
    }, onError: expectAsync1((Exception e) {
      expect(e is ConfigurationError, true);
      expect(e.toString(),
  ...
  test('config could not be loaded', () {
    runZoned(() {
      CidaasLoginProvider.checkAndLoadConfig(
          configPath: 'assets/cidaas_config_NotExisting_123456789.json');
    }, onError: expectAsync1((Exception e) {
      expect(e is ConfigurationError, true);
      expect(e.toString(),
  ...
  test('config is empty', () {
    runZoned(() {
      CidaasLoginProvider.checkAndLoadConfig(
          configPath: 'assets/cidaas_config_empty.json');
    }, onError: expectAsync1((Exception e) {
      expect(e is ConfigurationError, true);
      expect(e.toString(),
          'ConfigurationError: Could not load cidaas config from path: assets/cidaas_config_empty.json');
    }));
  }); //End of mock group
  ...
  test('well-known openIdConfiguration response parse error', () {
    runZoned(() {
      ...
      when(httpMock.get('https://baseUrl.de/.well-known/openid-configuration',
              headers: anyNamed('headers')))
          .thenAnswer(
              (_) => Future<http.Response>(() => http.Response('.', 200)));
      //Enable mock usage
      HTTPHelper(httpClient: httpMock);
      CidaasLoginProvider.checkAndLoadConfig();
    }, onError: expectAsync1((Exception e) {
  ...
  test('well-known openIdConfiguration response is null', () {
    runZoned(() {
      ...
      when(httpMock.get('https://baseUrl.de/.well-known/openid-configuration',
              headers: anyNamed('headers')))
          .thenAnswer(
              (_) => Future<http.Response>(() => http.Response('', 500)));
      //Enable mock usage
      HTTPHelper(httpClient: httpMock);
      CidaasLoginProvider.checkAndLoadConfig();
    }, onError: expectAsync1((Exception e) {
      ...
      expect(
          e.toString().contains(
              'WellKnownOpenIdConfigLoadError: Response from well-known endpoint https://baseUrl.de/.well-known/openid-configuration is null!'),
          true);
    }));
  });
}
