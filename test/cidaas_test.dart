  ...
  group('Cidaas abstract class tests, user is logged in & correct config:', () {
    ...
    setUp(() {
      ...
      when(storage.write(key: anyNamed('key'), value: anyNamed('value')))
          .thenReturn(null);
      when(storage.read(key: AuthStorageHelper.ACCESS_TOKEN)).thenAnswer(
          (_) => Future<String>(() => AuthStorageHelper.ACCESS_TOKEN));
    ...
    testWidgets('Cidaas starts with LoggedOut Screen',
      ...
      final Finder loggedOutScreenFinder = find.text('Login');
...
class MyCidaasImpl extends Cidaas {
  ...
  Widget getLoggedInScreen({TokenEntity tokenEntity}) {
    return const Text('LoggedInScreen');
  }
