...
void main() {
  ...
  group(
      'authenticationBloc tests with mocked secure storage, user not logged in:',
      () {
    ...
    test('initial state is correct', () {
      expect(
          authenticationBloc.initialState, AuthenticationHasLoggedOutState());
    });
    ...
    test('login event', () {
      ...
      expect(loggedInEvent.toString(),
          'AuthenticationLoggedInEvent { tokenEntity: TokenEntity {access_token: accessToken, id_token: idToken, sub: sub, refresh_token: refreshToken} }');
    });
  ...
  group(
      'authenticationBloc tests with mocked secure storage, user is logged in:',
      () {
    AuthenticationBloc authenticationBloc;
    TokenEntity tokenEntity;
    ...
    setUp(() {
      ...
      when(storage.write(key: ACCESS_TOKEN, value: ACCESS_TOKEN))
          .thenReturn(null);
      when(storage.write(key: SUB, value: SUB)).thenReturn(null);
      when(storage.write(key: ID_TOKEN, value: ID_TOKEN)).thenReturn(null);
      when(storage.write(key: REFRESH_TOKEN, value: REFRESH_TOKEN))
          .thenReturn(null);
      when(storage.read(key: AuthStorageHelper.ACCESS_TOKEN))
          .thenAnswer((_) => Future<String>(() => testToken));
      when(storage.read(key: AuthStorageHelper.SUB))
          .thenAnswer((_) => Future<String>(() => AuthStorageHelper.SUB));
      when(storage.read(key: AuthStorageHelper.ID_TOKEN))
          .thenAnswer((_) => Future<String>(() => AuthStorageHelper.ID_TOKEN));
      when(storage.read(key: AuthStorageHelper.REFRESH_TOKEN)).thenAnswer(
          (_) => Future<String>(() => AuthStorageHelper.REFRESH_TOKEN));
      authenticationBloc = AuthenticationBloc(secureStorage: storage);
      tokenEntity = TokenEntity(
    ...
    test('test Authentication from StartedEvent to successful', () {
      AuthenticationSuccessState authSuccess =
          AuthenticationSuccessState(tokenEntity: tokenEntity);
      final expectedResponse = [
        AuthenticationHasLoggedOutState(),
        AuthenticationInProgressState(),
    ...
    test('test AuthenticationLogOutEvent', () {
      ...
      Future.delayed(Duration(milliseconds: 50)).then(
          (value) => verify(storage.delete(key: anyNamed('key'))).called(4));
    });
  }); //End of mock group user is logged in

  test(
      'AuthenticationBloc is using a new instance of FlutterSecureStorage if no storage is provided',
      () {
    AuthenticationBloc authenticationBloc;
    runZoned(() {
